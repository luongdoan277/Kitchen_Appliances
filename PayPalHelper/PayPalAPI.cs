...
namespace Kitchen_Appliances.PayPalHelper
{
    public class PayPalAPI
    {
        ...
        public async Task<PayPalPaymentExecutedResponse> executedPayment(string paymentId, string payerId)
        {
            try
            {
                HttpClient http = GetPaypalHttpClient();
                PayPalAccessToken accessToken = await GetPayPalAccessTokenAsync(http);
                return await ExecutePaypalPaymentAsync(http, accessToken, payerId, paymentId);
            }
            catch (Exception ex)
            {
                Debug.WriteLine(ex, "Failed to login to PayPal");
        ...
        private async Task<PayPalPaymentCreatedResponse> CreatePaypalPaymentAsync(HttpClient http,PayPalAccessToken accessToken, decimal total, string currency)
        {
            ...
            var payment = JObject.FromObject(new
            {
                ...
                redirect_urls = new
                {
                    returl_url = configuration["PayPal:returnUrl"],
